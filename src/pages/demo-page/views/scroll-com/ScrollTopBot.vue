<template>
  <div class="scroll co-f1 co-flex co-ver">
    <header-com isBack :title="title"></header-com>
    <div class="co-f1 co-flex">
      <scroll-list ref = "scroll" pull-down-refresh pull-up-load :page-size = "pageSize" @show-data = "showData">
        <div v-for="item in dataList" :key="item" class="co-bd-b co-pd-a08 co-bg-0 co-flex co-jsb">
          我是下拉刷新上拉加载的列表<span>{{item}}</span>
        </div>
      </scroll-list>
    </div>
  </div>
